...
export const findUserByUsername = async ({ username }) => {
  const result = await db.query('SELECT * FROM users WHERE username = $1 AND is_verified = true' , [username]);
...
export const findEmailAndToken = async ( email, token ) => {
  const result = await db.query('SELECT * FROM users WHERE email = $1 AND email_token = $2', [email, token]);

  return result.rows[0];
};

export const updateUserVerificationStatus = async (userId, isVerified) => {
    const result = await db.query('UPDATE users SET is_verified = $1 WHERE id = $2', [isVerified, userId]);
